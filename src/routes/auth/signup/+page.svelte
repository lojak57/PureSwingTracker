<script lang="ts">
  ...
  const handleOAuthSignup = async (provider: 'google' | 'apple') => {
    ...
    if (!result.success) {
      error = result.error || 'OAuth signup failed';
      isLoading = false;
    }
  };
</script>
  ...
  <div class="sm:mx-auto sm:w-full sm:max-w-md">
    <div class="text-center">
      ...
      <p class="mt-2 text-sm text-gray-600">
        Already have an account? <a href="/auth/login" class="font-medium text-primary-600 hover:text-primary-500">
          Sign in
        </a>
      </p>
    </div>
  ...
  <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
    <div class="card py-8 px-4 shadow sm:rounded-lg sm:px-10">
      ...
      <form on:submit|preventDefault={handleSignup} class="space-y-6">
        ...
        <div>
          <label for="handicap" class="block text-sm font-medium text-gray-700">
          ...
          <div class="mt-1">
            <input
              ...
              max="54"
              step="0.1"
              bind:value={credentials.handicap}
              class="input-field"
              placeholder="Enter your handicap"
            />
          </div>
          <p class="mt-1 text-xs text-gray-500">
            This helps us provide better personalized coaching
          </p>
        </div>
      ...
      <div class="mt-6">
        ...
        <div class="mt-6 grid grid-cols-2 gap-3">
          ...
          <button
            type="button"
            on:click={() => handleOAuthSignup('apple')}
            disabled={isLoading}
            class="w-full inline-flex justify-center py-2 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50"
          >
            <svg class="w-5 h-5" viewBox="0 0 24 24">
